...
int main(int argc, char *argv[]) {
  ...
  /* set socket options */
  if (setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &sock_opt,
                 sizeof(sock_opt)) < 0)
    error("Error: set SO_REUSEADDR option");
  if (setsockopt(listen_fd, IPPROTO_TCP, TCP_NODELAY, &sock_opt,
                 sizeof(sock_opt)) < 0)
  ...
  if (cong != DEFAULT) {
    ...
    if (setsockopt(listen_fd, IPPROTO_TCP, TCP_CONGESTION, cong_str,
                   strlen(cong_str)) < 0)
  ...
  if (bind(listen_fd, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr)) <
      0)
    error("Error: bind");
  ...
  if (daemon_mode) {
    ...
    if (pid < 0)
      exit(EXIT_FAILURE);
    /* if we get the process ID of the child process, then we can exit the
     * parent process */
    if (pid > 0) {
  ...
  while (1) {
    ...
    *sockfd_ptr = accept(listen_fd, (struct sockaddr *)&cli_addr, &len);
    if (*sockfd_ptr < 0) {
      ...
      free(sockfd_ptr);
      error("Error: accept");
    } else if (pthread_create(&serv_thread, NULL, handle_connection,
                              (void *)sockfd_ptr) < 0) {
      close(listen_fd);
      free(sockfd_ptr);
